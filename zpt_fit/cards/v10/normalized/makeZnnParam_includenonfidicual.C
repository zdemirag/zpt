void makeZnnParam_includenonfidicual(
  int dependentBin=1,
  bool debug=false
) {
  ...
  for(int i=0; i<6; i++) { // gen PT bins
    ...
    paramStr+=Form("(1-@%d)*(%.3f",var,weight); var+=1; vars.push_back(Form("RDY_%d",i));
  ...
  for(int i=0; i<6; i++) { if(i==dependentBin) continue; printf("RDY_%d extArg 1.0 [0.0,2.0]\n", i); }
  for(int i=0; i<6; i++) { 
    if(i==dependentBin) 
      printf("%s\n",paramStr.Data());
    else
      printf("FR%d rateParam * zcat%d @0 RDY_%d\n", i, i, i); // zcat0 is non-fiducial in Z(vv) analysis
  }

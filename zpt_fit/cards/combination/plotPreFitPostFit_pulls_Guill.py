...
def plotPreFitPostFit(region,category,sb=False,setPull=True):
  ...
  folder = "/home/zdemirag/cms_2017/zpt/CMSSW_8_1_0/src/analysis/zpt_fit/cards/combination/"

  f_data = TFile(folder+"/datacard_Pt_mm.root")
  h_data = f_data.Get("histo_Data")
  print "INTEGRAL", h_data.Integral()

  f_mlfit = TFile(folder+'fitDiagnostics.root','READ')

  channel = {"dimuon":category+"_dimu", "signal":category+"_signal", "dielectron":category+"_diel"}
  mainbkg = {"dimuon":"DY", "signal":"DY", "dielectron":"DY"}

  print "shapes_prefit/"+category+"/DY_0"

  h_postfit_sig = f_mlfit.Get("shapes_fit_s/"+category+"/total_background")
  h_prefit_sig  = f_mlfit.Get("shapes_prefit/"+category+"/total_background")

  ##prefit
  h_signal0_pre = f_mlfit.Get("shapes_prefit/"+category+"/DY_0")
  h_signal1_pre = f_mlfit.Get("shapes_prefit/"+category+"/DY_1")
  h_signal2_pre = f_mlfit.Get("shapes_prefit/"+category+"/DY_2")
  h_signal3_pre = f_mlfit.Get("shapes_prefit/"+category+"/DY_3")
  h_signal4_pre = f_mlfit.Get("shapes_prefit/"+category+"/DY_4")
  ...
  ##postfit
  h_signal0 = f_mlfit.Get("shapes_fit_s/"+category+"/DY_0")
  h_signal1 = f_mlfit.Get("shapes_fit_s/"+category+"/DY_1")
  h_signal2 = f_mlfit.Get("shapes_fit_s/"+category+"/DY_2")
  h_signal3 = f_mlfit.Get("shapes_fit_s/"+category+"/DY_3")
  h_signal4 = f_mlfit.Get("shapes_fit_s/"+category+"/DY_4")
  ...
  # Pre-Fit
  h_prefit = {}
  h_prefit['total'] = f_mlfit.Get("shapes_prefit/"+category+"/total")
  print "shapes_prefit/"+category+"/total"
  ...
  for process in processes:
    h_prefit[process] = f_mlfit.Get("shapes_prefit/"+category+"/"+process)
  ...
  # Post-Fit
  h_postfit = {}
  h_postfit['totalsig'] = f_mlfit.Get("shapes_fit_s/"+category+"/total")
  h_postfit['total'] = f_mlfit.Get("shapes_fit_s/"+category+"/total")
  ...
  h_postfit['totalv2'] = f_mlfit.Get("shapes_fit_s/"+category+"/total_background")
  ##Adding Signal
  if region is 'signal':
    h_postfit['totalv2'].Add(f_mlfit.Get("shapes_fit_s/"+category+"/total_signal"))

  print "shapes_fit_s/"+category+"/total_background"
  ...
  for process in processes:
    print "\n\n",process
    h_postfit[process] = f_mlfit.Get("shapes_fit_s/"+category+"/"+process)
  ...
  if region in 'signal' :
    ...
    legend.AddEntry(h_signal0, "Z(#ell#ell)+jets", "f")
    legend.AddEntry(h_postfit['Res'], "Resonant", "f")
    legend.AddEntry(h_postfit['NonFid'], "Nonfidical", "f")
    legend.AddEntry(h_postfit['VV'], "Dibosons", "f")
  ...
  #if category is '2' or category is '6':
  ...
  folder2 ="/afs/cern.ch/user/z/zdemirag/www/zpt/panda/combination/"
  c.SaveAs(folder2+"/Zll_prefit_postfit_"+region+".pdf")
  c.SaveAs(folder2+"/Zll_prefit_postfit_"+region+".png")
  c.SaveAs(folder2+"/Zll_prefit_postfit_"+region+".C")
  c.SaveAs(folder2+"/Zll_prefit_postfit_"+region+".root")


plotPreFitPostFit("dimuon","ch1_ch2", setPull=True)

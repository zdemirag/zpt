...
def abcd(filename,mc=True):
    ...
    common = "(fabs(jot1EtaUp)<=2.4 && metFilter==1 && jot1PtUp>100 && fabs(jot1EtaUp)<2.4 && jet1IsTight==1 && nTau==0 && jetNMBtags==0 && nLooseLep==0 && nLoosePhoton==0 && fabs(calomet-pfmet)/pfmet<0.5"
    #fabs(jot1Eta)<=2.4

    if filename is "JetHT":
        print "analyzing JetHT dataset therefore, the HT 900 requirement is added:"
        common = "(metFilter==1 && jot1PtUp>100 && fabs(jot1EtaUp)<2.4 && jet1IsTight==1 && nTau==0 && jetNMBtags==0 && nLooseLep==0 && nLoosePhoton==0 && fabs(calomet-pfmet)/pfmet<0.5 && barrelHT>1000 && (trigger&16)!=0"
    ...
    selection_a = common + " && pfmetUp<=250 && dphipfmet<=0.5 )" 
    selection_b = common + " && pfmetUp>250  && dphipfmet<=0.5 )"
    selection_c = common + " && pfmetUp<=250 && dphipfmet>0.5 )"
    selection_d = common + " && pfmetUp>250  && dphipfmet>0.5 )"
    selection_inclow  = common + "&& pfmetUp>50  && dphipfmet<=0.5 )"
    selection_inchigh = common + "&& pfmetUp>50  && dphipfmet>0.5 )"
    ...
    intree.Draw("pfmetUp>>ha"+filename,selection_a+common_weights,"goff")
    intree.Draw("pfmetUp>>hb"+filename,selection_b+common_weights,"goff")
    intree.Draw("pfmetUp>>hc"+filename,selection_c+common_weights,"goff")
    intree.Draw("pfmetUp>>hd"+filename,selection_d+common_weights,"goff")

    intree.Draw("pfmetUp>>hinc_high"+filename,selection_inchigh+common_weights,"goff")
    intree.Draw("pfmetUp>>hinc_low"+filename,selection_inclow+common_weights,"goff")
    ...
    intree.Draw("dphipfmet:pfmetUp>>h2d"+filename,common+")"+common_weights,"goff")
...
def main():
    ...
    outfile = TFile("qcd_abcd_Up.root","recreate")

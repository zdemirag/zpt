...
ptbinshigh = [250,275,300,350,400,450,500,650,800,1150,1500] 
#[ 250.0, 300.0, 350.0, 400.0, 500.0, 750.0, 1000.0, 1500.0]
ptbins = [100,125,150,175,200,225,250,275,300,350,400,450,500,650,800,1150,1500] 
#250.0, 300.0, 350.0, 400.0, 500.0, 750.0, 1000.0, 1500.0]
...
def abcd(filename,mc=True):
    ...
    infile = TFile("/desktop/05a/zdemirag/v_004_11jetht/"+filename+".root","read") 
    ...
    common = "(fabs(jot1EtaDown)<=2.4 && metFilter==1 && jot1PtDown>100 && fabs(jot1EtaDown)<2.4 && jet1IsTight==1 && nTau==0 && jetNMBtags==0 && nLooseLep==0 && nLoosePhoton==0 && fabs(calomet-pfmet)/pfmet<0.5"
    #fabs(jot1Eta)<=2.4

    if filename is "JetHT":
        print "analyzing JetHT dataset therefore, the HT 900 requirement is added:"
        common = "(metFilter==1 && jot1PtDown>100 && fabs(jot1EtaDown)<2.4 && jet1IsTight==1 && nTau==0 && jetNMBtags==0 && nLooseLep==0 && nLoosePhoton==0 && fabs(calomet-pfmet)/pfmet<0.5 && barrelHT>1000 && (trigger&16)!=0"
    ...
    selection_a = common + " && pfmetDown<=250 && dphipfmet<=0.5 )" 
    selection_b = common + " && pfmetDown>250  && dphipfmet<=0.5 )"
    selection_c = common + " && pfmetDown<=250 && dphipfmet>0.5 )"
    selection_d = common + " && pfmetDown>250  && dphipfmet>0.5 )"
    selection_inclow  = common + "&& pfmetDown>50  && dphipfmet<=0.5 )"
    selection_inchigh = common + "&& pfmetDown>50  && dphipfmet>0.5 )"
    ...
    intree.Draw("pfmetDown>>ha"+filename,selection_a+common_weights,"goff")
    intree.Draw("pfmetDown>>hb"+filename,selection_b+common_weights,"goff")
    intree.Draw("pfmetDown>>hc"+filename,selection_c+common_weights,"goff")
    intree.Draw("pfmetDown>>hd"+filename,selection_d+common_weights,"goff")

    intree.Draw("pfmetDown>>hinc_high"+filename,selection_inchigh+common_weights,"goff")
    intree.Draw("pfmetDown>>hinc_low"+filename,selection_inclow+common_weights,"goff")
    ...
    intree.Draw("dphipfmet:pfmetDown>>h2d"+filename,common+")"+common_weights,"goff")
...
def main():
    ...
    outfile = TFile("qcd_abcd_Down.root","recreate")
